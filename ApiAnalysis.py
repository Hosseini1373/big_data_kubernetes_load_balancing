''''
This is a Python script that performs a load test on a Kubernetes API server. 
...
def requesting(results_queue):
    ...
    while True:
        try:
            response = requests.get('https://test.questionpdf.init-lab.ch/query?query=who%20is%20john', timeout=max_time)
...
def visualize_results(success_count, timeout_count, average_response_time, success_response_times, conc,iteration):
    ...
    figures = []
    current_figure = 0
    if not os.path.exists('images/K8/'):
        os.makedirs('images/K8/')
    if success_count > 0:
        ...
        plt.savefig(f'images/K8/api_test_results_{iteration}_{conc}.png')
        ...
        plt.savefig(f'images/K8/response_time_distribution_{iteration}_{conc}.png')
        ...
        plt.savefig(f'images/K8/response_time_box_plot_{iteration}_{conc}.png')
        ...
        plt.savefig(f'images/K8/response_time_percentiles_{iteration}_{conc}.png')
...
if __name__ == '__main__':
    for conc in [4,8,16,32,64]:
        ...
        # Write results to a CSV file
        with open(f'images/K8/results_{conc}.csv', 'w', newline='') as f:
            writer = csv.writer(f)
        ...
        create_boxplots(f'images/K8/results_{conc}.csv')